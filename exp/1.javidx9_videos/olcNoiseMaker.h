...
public:
	...
	{
		/* ENUMERATE: checks the available devices in windows.
			waveOutGetNumDevs: number of output devices
			WAVEOUTCAPS: structure which describes the capabilities of a 
			waveform-audio device. Contains information such as supported 
				format, manufacturer Id, product name, number of channels (mono
				1, stereo 2) and more.
			waveOutGetDevCaps: retrieves the capabilities of a given 
				waveform-audio output device. Consider that first parameter is 
				the device  number (unsigned integer), second parameter is a 
				WAVEOUTPCAPS  structure which will be filled when the devices 
				is an audio output, and third is always the number of bytes of
				the WAVEOUTCAPS structure 
		*/
		...
		/* Find available waveform-audio output devices. */
		WAVEOUTCAPS woc; 
		for (int n = 0; n < nDeviceCount; n++){
			/* Check whether the devices n is available for audio output */
			if (waveOutGetDevCaps(n, &woc, sizeof(WAVEOUTCAPS)) == S_OK) {
				sDevices.push_back(woc.szPname);  // wstring of 32 characters
			}
		}
		return sDevices;
	}
